...
if ($_version == 'v2' && $_api == 'api' && $_endpoint == 'historia') {
    switch ($_metodo) {
        case 'GET':
            if ($_header == $_token_get) {
                include_once 'controller.php';
                $control = new ControladorHistoria();
                if (isset($valorId)) {
                    $lista = $control->getHistoriaById($valorId);
                } else {
                    $lista = $control->getAllHistorias();
                }
        ...
        case "DELETE":
            if ($_header == $_token_delete) {
                if (isset($valorId)) {
                    include_once "controller.php";
                    $control = new ControladorHistoria();
                    $rs = $control->deleteHistoria($valorId);
                    if ($rs) {
                        http_response_code(200);
                        echo json_encode(["data" => $rs]);
                    } else {
